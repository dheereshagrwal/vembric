...
<Note>
  ...
  your code before contacting Vembric support.
</Note>
...
Here is a list of the different categories of status codes returned by the Vembric API. Use these to understand if a request was successful.
...
<Row>
  <Col>
    ...
    Whenever a request is unsuccessful, the Vembric API will return an error response with an error type and message. You can use this information to understand better what has gone wrong and how to fix it. Most of the error messages are pretty helpful and actionable.

    Here is a list of the two error types supported by the Vembric API — use these to understand what you have done wrong.
  ...
  <Col>
    ...
    ```bash {{ title: "Error response" }}
    {
      ...
      "documentation_url": "https://vembric.chat/docs/errors/api_error"
    }
    ```
